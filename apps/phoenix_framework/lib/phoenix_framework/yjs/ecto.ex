defmodule PhoenixFramework.Yjs.Ecto do
  defmacro __using__(opts) do
    ...
    quote do
      ...
      def insert_update(doc_name, value) do
        @repo.insert(%@schema{doc_name: doc_name, value: value, version: :v1})
      end

      def get_state_vector(doc_name) do
        query =
          from(y in @schema,
            where: y.doc_name == ^doc_name and y.version == :v1_sv,
            select: y
          )
      ...
      defp get_updates(doc_name) do
        query =
          from(y in @schema,
            where: y.doc_name == ^doc_name and y.version == :v1,
            select: y,
      ...
      defp flush_document(doc_name, updates, sv, clock) do
        @repo.insert(%@schema{doc_name: doc_name, value: updates, version: :v1})
      ...
      defp clear_updates_to(doc_name, to) do
        query =
          from(y in @schema,
            where: y.doc_name == ^doc_name and y.inserted_at < ^to
          )
