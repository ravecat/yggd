...
describe("TodosSortMenu", () => {
  ...
  test("adds a default sorting rule and applies it to the URL", async () => {
    const { TodosSortMenu } =
      await import("~/app/todos/[boardId]/_components/todos-sort-menu");
    ...
    await waitFor(() => {
      expect(replaceMock).toHaveBeenCalledWith(
        "/todos/board-1?filter=roadmap&sort=-priority",
        { scroll: false },
      );
    });
  });
  ...
  test("removes an existing sorting rule while preserving other query params", async () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Remove Title sort rule" }),
    );

    await waitFor(() => {
      expect(replaceMock).toHaveBeenCalledWith(
        "/todos/board-1?filter=roadmap",
        {
          scroll: false,
        },
      );
