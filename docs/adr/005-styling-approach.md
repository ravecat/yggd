...
styling strategy must balance visual consistency with framework-specific idioms.

## Decision drivers

- Visual consistency: apps should look recognizably similar across implementations.
- Framework-native patterns: each app should use its framework's preferred styling approach.
- Framework alignment: preserve each framework's native styling model.
...
- Bad, because reduces use of framework-native styling patterns.

### Option B

- Description: Fully framework-native (independent styling per app)
- Good, because preserves idiomatic framework-specific styling.
- Bad, because apps may look completely different, reducing cross-app consistency.
...
- Each app uses its framework's styling primitives: Svelte scoped styles, Vue SFC styles, CSS modules, etc.
- Easy to maintain consistent look without coupling implementations.

### Negative consequences

- Less Tailwind reuse across frameworks (only Next.js uses it currently).
